...
def get_api():
    ...
    token = token.strip()  # ❗ критично

    return f"https://api.telegram.org/bot{token}"

# 🔹 send inline keyboard
def send_inline(chat_id, text, keyboard):
    url = f"{get_api()}/sendMessage"
...
async def webhook(request: Request):
    ...
    if "callback_query" in data:
        ...
        if action == "BUY":
            send_message(chat_id, "Купівля вибрана")
        elif action == "SELL":
            send_message(chat_id, "Продаж вибраний")
        elif action == "EXPENSE":
            send_message(chat_id, "Витрати вибрані")
